...
distance=lond(1:192,80,10);
%%distance=lond(1:192,1);%start=65, count=116 thus end=65+116-1=180
%ww=ncread(dataname,'W');
%load("ww.txt");
%ww=ncread(dataname,'T',[1,80,46,10],[192,1,1,1],[1,1,1,1]);
ww = sin(5*distance)+sin(20*distance);
%%ww=ncread(dataname,'W',[1,70,42,15],[192,1,1,1],[1,1,1,1]);
%%ww=ncread(dataname,'T',[1,80,46,10],[192,1,1,1],[1,1,1,1]);
disp('ww');
...
%YtickLabel=0:32:288; %and modify ylim=[0,60];
...
[c11,h11] = contourf(distance,log(wavelength),real_wave,levels); 
...
set(title('c) Global WPS'),'FontName','Times New Roman','FontSize',18,'Color','b')
colorDepth = 1000;
colormap(jet(colorDepth));
set(gca,'FontName','Times New Roman','FontSize',18);
set(findobj('FontSize',18),'FontSize',figure_FontSize);
